            ...
            if (cmd == Command::SEND_PARCEL) {
                ...
                if (type != UserType::CUSTOMER) {
                    g_logger.warning("RechargeBalance command: Permission denied for user: " + 
                        m_currentUser);
                    return buildResponse("ERROR", {"Only customers can recharge balance"});
                }
...
std::string ClientHandler::buildResponse(const std::string& status, const std::vector<std::string>& data, ErrorCode code) {
...
std::string ClientHandler::handleLogout(const std::vector<std::string>& args) {
    ...
    if (ec == ErrorCode::SUCCESS) {
        ...
        return buildResponse("ERROR", {"User not logged in"}, ec);
        ...
        return buildResponse("ERROR", {"Logout failed"}, ec);
    }
}
...
std::string ClientHandler::handleLogin(const std::vector<std::string>& args) {
    ...
    if (ec == ErrorCode::SUCCESS) {
        ...
        return buildResponse("OK", {m_currentUser, std::to_string(typeInt)});
    } else {
...
std::string ClientHandler::handleQueryParcel(const std::vector<std::string>& args) {
    ...
    if (type == UserType::ADMINISTRATOR) {
        ...
        auto tmp2 = m_system->queryParcels(parcelId, sender, receiver, m_currentUser, status, startTime, endTime);
        parcels = tmp2.second;
    } else {
        if (sender.empty() && receiver.empty()) {
            auto q1 = m_system->queryParcels(parcelId, m_currentUser, "", "", status, startTime, endTime);
            auto q2 = m_system->queryParcels(parcelId, "", m_currentUser, "", status, startTime, endTime);
            parcels = q1.second;
...
std::string ClientHandler::handleQueryBalance(const std::vector<std::string>& args) {
    auto [ec, balance] = m_system->getUserBalance(m_currentUser);
    if (ec != ErrorCode::SUCCESS) {

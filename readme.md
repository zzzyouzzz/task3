...
| 实践 | 说明 |
|------|------|
| **分层架构** | 表现层 / 控制层 / 业务层 / 数据层职责分离清晰 |
| **面向接口编程** | User / Parcel 基类 + 多态派生（NormalParcel / FragileParcel / BookParcel） |
| **CMake 模块化** | Common 静态库被所有可执行文件共享，避免代码重复 |
| **安全数据解析** | `parseInt` / `parseDouble` / `parseLongLong` 带完整错误校验 |
| **异常隔离** | 请求处理外层包裹 `try-catch`，防止单个请求异常导致服务崩溃 |
| **RAII 资源管理** | socket、文件句柄、内存均在析构函数中自动释放 |
| **日志追踪** | 每个请求分配唯一 ID，日志包含完整请求-响应链路 |
| **优雅关闭** | 支持 Ctrl+C / Ctrl+Break 信号处理，安全释放资源 |
...
| **客户端响应读取** | 目前单次 `recv()` 最多读取 4096 字节且无累积缓冲区，查询等大数据响应若被 TCP 分片将导致截断，需增加与服务器端相同的 `\n` 边界检测 + 缓冲重组机制 |

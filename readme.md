...
| 方向 | 说明 |
|------|------|
| **密码安全** | 当前密码明文存储，建议引入 SHA-256 + Salt 哈希 |
| **客户端响应读取** | 单次 `recv()` 最多 4096 字节且无累积缓冲区，查询等大数据响应若被 TCP 分片将导致截断，需增加 `\n` 边界检测 + 缓冲重组 |
| **send 完整性** | 未处理 `send()` 部分发送场景；大数据响应可能需要多次发送 |
| **handleClientRequest 公平性** | `while(true)` 循环连续处理同一连接的所有报文，不返回 select 主循环，可能导致其他连接被饿死。建议每次调用限制处理一个请求，或设置单次最大处理数量 |
| **裸指针管理** | `m_users` / `m_parcels` 存储裸指针，手动 `new`/`delete`，异常抛出时可能泄漏。建议改用 `std::unique_ptr` 自动管理生命周期 |
| **saveData 无事务保护** | 每次写操作全量写回三个文件，若写入过程中途失败（如磁盘满）会导致启动时数据状态不一致。建议写临时文件后 `rename()` 原子替换，或引入版本号校验 |
| **连接保活** | 缺少空闲超时断开机制，僵尸连接会持续占用资源 |
| **Linux 兼容性** | 非阻塞设置与错误码判断的跨平台实现可进一步完善 |
| **测试数据路径脆弱** | `dataset_tests.cpp` 的 `findDataDir()` 通过枚举 5 个候选路径定位数据目录，依赖当前工作目录，在 CMake 构建目录下运行时容易猜测失败 |
| **QoS/重连** | 缺少断线重连和请求超时机制 |
| **并发演进** | 架构已预留上下文设计，未来可升级为多线程处理 |

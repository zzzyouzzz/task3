...
private:
    ...
    // 读取浮点数
    bool readDouble(const std::string& prompt, double& value, double minValue, double maxValue) const;  
    // 读取字符串
    bool readString(const std::string& prompt, std::string& value) const;
    // 读取时间（YYYY-MM-DD HH:MM:SS 格式）
    bool readTime(const std::string& prompt, time_t& value) const;
